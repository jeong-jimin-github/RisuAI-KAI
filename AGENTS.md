...
| `DEPLOYMENT.md` | SSH remote deployment and agent reporting rule |
...
## Deployment & Agent Reporting Workflow

Whenever changes are made to the codebase (frontend, backend, styles, assets), the agent must:
1. Run tests and production builds (`pnpm test`, `pnpm build`, and `pnpm -C kai/server build` if server files modified).
2. Execute `python scripts/deploy.py` to deploy to SSH `u0_a361@192.168.219.101:8022` (`/data/data/com.termux/files/home/risuai-kai`).
3. Verify remote service health.
4. Report modified files, build/deploy status, and live URL (`https://risuai.work.gd/`) to the user in the response.
