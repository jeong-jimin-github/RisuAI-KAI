...
export async function sendChat(chatProcessIndex = -1,arg:{
    ...
    const authorNoteText = resolveAuthorNoteText(currentChat.note, currentChar.postHistoryInstructions)

    if(authorNoteText){
        unformated.authorNote.push({
            role: 'system',
            content: risuChatParser(authorNoteText, {chara: currentChar})
        })
    }
        ...
        for(const card of template){
            switch(card.type){
                ...
                case 'authornote':{
                    ...
                    if(pmt.length === 0 && card.defaultText){
                        pmt.push({
                            role: 'system',
                            content: risuChatParser(card.defaultText, {chara: currentChar})
                        })
                    }
                    applyPromptBlockRole(pmt, card.role2)
                    if(card.innerFormat && pmt.length > 0){
                ...
                case 'cot':{
                    ...
                    if(card.type2 === 'globalNote'){
                        ...
                        content = resolveTemplateMainPrompt(content, currentChar.systemPrompt ? positionParser(currentChar.systemPrompt, posType) : undefined)
                        content = (risuChatParser(content, {chara: currentChar, role: card.role}))
                    }
                    else{
                        content = risuChatParser(content, {chara: currentChar, role: card.role})
