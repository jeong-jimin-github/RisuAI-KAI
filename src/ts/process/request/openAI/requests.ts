...
import { addFetchLog, fetchNative, globalFetch, isSameOriginUrl, textifyReadableStream } from "src/ts/globalApi.svelte"
...
import { attachKaiChatTrace, KAI_ACTIVE_CHAT_ID_KEY } from '../kaiTrace'
export { requestOpenAIResponseAPI, __testResponsesAPI } from './responses'

export type OpenAIResponseMetadataListener = (metadata: { url: string; headers: Record<string, string> }) => void
export const openAIResponseMetadataListeners = new Set<OpenAIResponseMetadataListener>()

export function notifyOpenAIResponseMetadata(url: string, headers: any) {
    if (!openAIResponseMetadataListeners.size) return
    ...
    for (const listener of openAIResponseMetadataListeners) {
        try {
            listener({ url, headers: headerMap })
        } catch (e) {
            console.error('Error in openAIResponseMetadataListener', e)
        }
    }
}
...
export async function requestOpenAI(arg:RequestDataArgumentExtended):Promise<requestDataResponse>{
    ...
    // KAI: tell our own gateway which hosted chat this generation belongs to, so
    // it can attribute usage and keep a completed reply when the browser drops
    // mid-stream. Only ever sent to a same-origin gateway.
    attachKaiChatTrace(body, {
        aiModel,
        chatId: (db as unknown as Record<string, unknown>)[KAI_ACTIVE_CHAT_ID_KEY],
        sameOrigin: isSameOriginUrl(replacerURL),
    })
    ...
    if(arg.useStreaming){
        ...
        if (da && da.headers) {
            notifyOpenAIResponseMetadata(replacerURL, da.headers)
        }
